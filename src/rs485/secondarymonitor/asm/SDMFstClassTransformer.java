						...
						MethodNode mn = new MethodNode(access, name, desc, signature, exceptions) {
							...
							public void visitTypeInsn(int opcode, String type) {
								if(changing == ASMState.WAITING && opcode == Opcodes.NEW && "mapwriter/region/SurfacePixels".equals(type)) {
									type = "rs485/secondarymonitor/mapwriter/SDMFstSurfacePixels";
							...
							public void visitMethodInsn(int opcode, String owner, String name, String desc) {
								if(changing == ASMState.CHANGING) {
									if(opcode == Opcodes.INVOKESPECIAL && "mapwriter/region/SurfacePixels".equals(owner) && "<init>".equals(name) && "(Lmapwriter/region/Region;Ljava/io/File;)V".equals(desc)) {
										owner = "rs485/secondarymonitor/mapwriter/SDMFstSurfacePixels";
									}
					...
					if(access == Opcodes.ACC_PUBLIC && name.equals("initGui") && desc.equals("()V")) { //TODO srgNames
						MethodNode mn = new MethodNode(access, name, desc, signature, exceptions) {
							...
							public void visitLabel(Label label) {
								if(tryCatch >= 2 && !codeAdded) {
									...
									super.visitTypeInsn(Opcodes.NEW, "rs485/secondarymonitor/gui/GuiButtonStartSndJVM");
									...
									super.visitMethodInsn(Opcodes.INVOKESPECIAL, "rs485/secondarymonitor/gui/GuiButtonStartSndJVM", "<init>", "(III)V");
						...
						MethodNode mn = new MethodNode(access, name, desc, signature, exceptions) {
							...
							public void visitCode() {
								...
								super.visitMethodInsn(Opcodes.INVOKESTATIC, "rs485/secondarymonitor/asm/ASMHookClass", "handleMainMenuButton", "(Lnet/minecraft/client/gui/GuiButton;)Z");
								Label l1 = new Label();
