package rs485.secondarymonitor.mapwriter;
...
public class SDMFstSurfacePixels extends SurfacePixels {
	...
	private void chunkChanged(int x, int z, int[] changedPixels) {
		((ClientProxy)MainProxy.proxy).sendConsolePacket(ConsolePacketHandler.getPacket(ChunkUpdatePacket.class).setRegionX(this.region.x).setRegionZ(this.region.z).setZoomLevel(this.region.zoomLevel).setDimension(this.region.dimension).setX(x).setZ(z).setUpdateData(changedPixels));
	}
	
	private void regionChanged() {
		((ClientProxy)MainProxy.proxy).sendConsolePacket(ConsolePacketHandler.getPacket(RegionUpdatePacket.class).setRegionX(this.region.x).setRegionZ(this.region.z).setZoomLevel(this.region.zoomLevel).setDimension(this.region.dimension).setUpdateData(pixels));
	}
